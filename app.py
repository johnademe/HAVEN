...
# ==================== DAILY SALES REQUIRED ====================
@app.route('/api/sales/daily-required')
@login_required
def daily_sales_required():
    ...
    try:
        ...
        # Get average daily sales from last 30 days (excluding today)
        cur.execute("""
            SELECT 
...
def summary_report():
    ...
    # today's credit from credits table (new)
    cur.execute("""
    ...
    return jsonify({
        ...
        "todays_credit": todays_credit['todays_credit']  # New field added
    })
...
def calendar_data():
    try:
        ...
        try:
            # If date has timezone info (like 2026-05-31T00:00:00+03:00)
            if 'T' in start:
                start_date = datetime.fromisoformat(start.replace('Z', '+00:00')).strftime('%Y-%m-%d')
            else:
                start_date = start[:10]  # Just take YYYY-MM-DD
            ...
            # Fallback: just take first 10 characters
